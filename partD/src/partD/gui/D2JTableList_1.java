...
//"이" 라고 검색시 "이" 와 동일한 단어만 출력
@SuppressWarnings("serial")
public class D2JTableList_1 extends JFrame {
	DefaultTableModel dm;
	JTable jt;
	String[] cols = {"English","Korean","Level","Write date"};
	
	public D2JTableList_1() {
		...
		btn.addActionListener(new ActionListener() {   //ActionListener 익명 구현 클래스
			...
			public void actionPerformed(ActionEvent e) {
				...
				for(int i=0;i<list.size();i++) {           //새로운 결과데이터 추가 보여주기
					Word temp = list.get(i);
					boolean flag;
					if(find.equals("")) flag =true;	//검색 단어 입력내용 없으면 flag 참(true)
					else 
						if(jc.getSelectedIndex() == 0)	//검색 기준이 영어일때 equals 실행
						flag = temp.getEnglish().equals(find);
						else							//검색 기준이 한글일때 equals 실행
